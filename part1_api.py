...
def get_dots(image, threshold):
    ...
    lights_indices = np.argwhere(threshold < c_image_converted)
    y_array = lights_indices[:, 0] + 10
    x_array = lights_indices[:, 1] + 10
...
def find_tfl_lights2(c_image: np.ndarray, **kwargs):
    ...
    dots_red_layer = find_tfl_by_layer(c_image, 0, 1000, 6400)

    dots = np.hstack([dots_green_layer, dots_red_layer])
...
def test_find_tfl_lights(image_path, json_path=None, fig_num=None):
    ...
    red_x, red_y, green_x, green_y = find_tfl_lights2(image, some_threshold=42)
